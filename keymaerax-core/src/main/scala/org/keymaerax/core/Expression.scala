...
case object ExpressionKind extends Kind {
...
/** Unit type of [[org.keymaerax.core.Nothing Nothing]] used as no argument. */
case object Unit extends Sort {
  override def toString = "Unit"
}

/** Sort of booleans: [[org.keymaerax.core.True True]], [[org.keymaerax.core.False False]]. */
...
case object Trafo extends Sort {
  override def toString = "Trafo"
}

/** Tuple sort for [[org.keymaerax.core.Pair Pair]]. */
final case class Tuple(left: Sort, right: Sort) extends Sort {
  override def toString: String = "(" + left + "," + right + ")"
}

/** User-defined object sort. */
final case class ObjectSort(name: String) extends Sort {
...
/**
 * The sort denoting the whole state space alias list of all variables as arguments \bar{x} (axioms that allow any
 * variable dependency)
 */
case object AnyArg extends Space {
  override def toString: String = "||"
}

/** The sort denoting a slice of the state space that does not include/depend on/affect variables `taboos`. */
final case class Except(taboos: immutable.Seq[Variable]) extends Space {
 ...
 *
 *   1. [[Term terms]] are of type [[org.keymaerax.core.Term]] of kind [[org.keymaerax.core.TermKind]]
 *   1. [[Formula formulas]] are of type [[org.keymaerax.core.Formula]] of kind [[org.keymaerax.core.FormulaKind]]
 *   1. [[Program hybrid programs]] are of type [[org.keymaerax.core.Program]] of kind
 *      [[org.keymaerax.core.ProgramKind]]
 *   1. [[DifferentialProgram differential programs]] are of type [[org.keymaerax.core.DifferentialProgram]] of kind
 *      [[org.keymaerax.core.DifferentialProgramKind]]
...
/** Real terms. Used for commonality instead of leaking outside the core, because inferable from sort */
...
final case class BaseVariable(name: String, index: Option[Int] = None, sort: Sort = Real) extends Variable {
 ...
 *
 * Differential symbols are also called differential variables, because they are symbolic but are also variables.
 */
final case class DifferentialSymbol(x: Variable) extends Variable with RTerm {
...
final case class Number(value: BigDecimal) extends AtomicTerm with RTerm
...
final case class Function(
  ...
  if (interpreted) {
    ...
    insist(
      StaticSemantics.freeVars(interp.get).isEmpty,
      "Function interpretation must not mention free variables: " + interp.get,
    )
    ...
    insist(
      signature.subsetOf(validDots.toSet),
...
final case class FuncOf(func: Function, child: Term) extends CompositeTerm with ApplicationOf {
...
/**
 * Unary Composite Real Terms, i.e. real terms composed of one real term. Used for commonality instead of leaking
 * outside the core, because inferable from sort.
 */
...
/**
 * `⎵`: Placeholder for formulas in uniform substitutions. Reserved nullary predicational symbol `_` for substitutions
 * is unlike ordinary predicational symbols by convention.
 */
...
final case class PredOf(func: Function, child: Term) extends AtomicFormula with ApplicationOf {
...
/**
 * Differential formula are differentials of formulas in analogy to differential terms (child)'. In theory they are only
...
/**
 * Hybrid programs of differential dynamic logic. Also includes hybrid games of differential game logic, which only
 * differ in the permitted mention of [[Dual]].
 *
 * Hybrid Programs are of the form
 *   - [[AtomicProgram]] atomic programs that are not composed of other programs
 *     - `x:=e` assignment as [[Assign]]([[Variable]],[[Term]]) and likewise differential assignment `x':=e` as
 *       [[Assign]]([[DifferentialSymbol]],[[Term]])
 *     - `x:=*` nondeterministic assignment as [[AssignAny]]([[Variable]]) and likewise nondeterministic differential
...
/**
 * Uninterpreted program constant symbol / game symbol, possibly limited to the given state space. The semantics of
 * ProgramConst symbol is looked up by the state, with the additional promise that taboos are neither free nor bound, so
 * the run does not depend on the value of taboos nor does the value of taboos change when space=Except(taboos).
 * @param space
...
final case class SystemConst(name: String, space: Space = AnyArg)
    extends NamedSymbol with AtomicProgram with SpaceDependent {
  override def asString: String = super.asString +
    (space match {
      case AnyArg => "{|^@|}"
...
final case class Assign(x: Variable, e: Term) extends AtomicProgram {
...
final case class AssignAny(x: Variable) extends AtomicProgram

/** ?cond test a formula as a condition on the current state. */
final case class Test(cond: Formula) extends AtomicProgram
...
final case class ODESystem(ode: DifferentialProgram, constraint: Formula = True) extends Program {

  /**
   * differentials could be allowed according to JAR'17, but the implementation is simplified when disallowed. But then
   * ODE axioms would need generalizations to q(x,x') and DS DI Cont need [x':=f(x)] generalizations.
   */
  insist(
    !StaticSemantics.isDifferential(constraint),
    "No differentials in evolution domain constraints {" + ode + " & " + constraint + "}",
  )
}
...
/**
 * Uninterpreted differential program constant, limited to the given state space. The semantics of arity 0
...
final case class AtomicODE(xp: DifferentialSymbol, e: Term) extends AtomicDifferentialProgram {
  ...
  insist(
    xp.x.isInstanceOf[BaseVariable] && !StaticSemantics.isDifferential(e),
    "Explicit-form differential equations expected, without any differentials on right-hand side: " + xp + "=" + e,
  )
}
...
object DifferentialProduct {
  ...
  def apply(left: DifferentialProgram, right: DifferentialProgram): DifferentialProduct = {
    insist(
      differentialSymbols(left).intersect(differentialSymbols(right)).isEmpty,
    ...
    (
      ...
      "No undetected duplicate differential equations when composing differential equations " + left + " and " + right +
        " to form " + reassociate(left, right),
    )
  ...
  /**
   * The list of all literal differential symbols in ODE. DifferentialProgramConst is counted as having no symbols just
   * yet, because its use is to check nonoverlap of present primed symbols.
   */
  ...
  private def differentialSymbols(ode: DifferentialProgram): immutable.List[DifferentialSymbol] = {
    ode match {
    ...
    (
      ...
      "StaticSemantics should agree since differential symbols only occur on the left-hand side of differential equations " +
        StaticSemantics.symbols(ode).toList.filter(x => x.isInstanceOf[DifferentialSymbol]),
    )
