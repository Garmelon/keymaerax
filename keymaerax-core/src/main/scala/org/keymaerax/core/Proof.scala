  ...
  def apply(newConsequence: Sequent, rule: Rule): Provable = {
    ...
    insist(
      ...
      "Rule " + rule + " was expected to justify\n" + this.conclusion.prettyString + "\n-----------------------------" +
        ...
        org.keymaerax.parser.FullPrettyPrinter(this.conclusion) + "\nFound:    " +
        rule(newConsequence).map(s => org.keymaerax.parser.FullPrettyPrinter(s)).mkString(", "),
    )
...
object Provable {
  ...
  private val trustedTools: immutable.List[String] = "org.keymaerax.tools.qe.MathematicaQETool" ::
    "org.keymaerax.tools.qe.Z3QETool" :: "org.keymaerax.tools.qe.BigDecimalQETool$" :: Nil
   ...
   *   |- ._0 = f<< P(._0,._1,...,._n) >>(._1,...,._n)  <->  P(._0,._1,...,._n)
   * }}}
   *
   * @param f
  ...
  // = java.lang.Class.forName("org.keymaerax.parser.FullPrettyPrinter").newInstance
  private val store: PrettyPrinter.PrettyPrinter = org.keymaerax.parser.FullPrettyPrinter
