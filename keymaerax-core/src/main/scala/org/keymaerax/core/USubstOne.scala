 ...
 * occurrences of a given predicate p(.) by a formula in (.). It can also replace all occurrences of a function symbol
 * f(.) by a term in (.) and all occurrences of a predicational / quantifier symbols C(-) by a formula in (-) and all
 * occurrences of program constant symbol b by a hybrid program.
 *
...
final case class USubstOne(subsDefsInput: immutable.Seq[SubstitutionPair]) extends (Expression => Expression) {
  ...
  def apply(e: Expression): Expression = e match {
    ...
    case f: Function => throw SubstitutionClashException(
        toString,
        "",
        e.toString,
        "",
  ...
  def applyAllTaboo(s: Sequent): Sequent =
    try { Sequent(s.ante.map(applyAllTaboo), s.succ.map(applyAllTaboo)) }
    catch { case ex: ProverException => throw ex.inContext(s.toString) }
  ...
  private def usubst(u: SetLattice[Variable], program: Program): (SetLattice[Variable], Program) = {
    program match {
      ...
      // optimized version:
      case Loop(a) =>
        val v = u ++ substBoundVars(a); val (w, ra) = usubst(v, a);
