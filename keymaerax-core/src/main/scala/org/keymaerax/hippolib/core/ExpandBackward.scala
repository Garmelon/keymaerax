...
case class ExpandBackward(names: Name*) extends BackwardTactic {
  ...
  override def runBackward(
    ...
    for (name <- names) chain = chain.joinBackward(ctx.expandBackward(chain.proof.premises.head.sequent, name))
    chain.proof
  }
}
