...
private[tools] object MathematicaNameConversion {
  ...
  def toMathematica(ns: NamedSymbol): MExpr = {
    val name: String = ns match {
      case Function(_, _, _, _, Some(_)) =>
        throw ConversionException("Name conversion of interpreted function symbols not allowed: " + ns.name)
   ...
   * between variables and functions (Symbol -> Variable, Expr with arguments -> Function). In each case, decomposes the
   * Mathematica name based upon the possible forms of the name:
   * {{{
  ...
  protected def convertFunctionDomain(args: Array[MExpr]): Sort = {
    ...
    args.map(convertFunctionDomain).reduceRightOption(Tuple.apply).getOrElse(Unit)
  }
  ...
  private def unmaskName(maskedName: String): (String, Option[Int]) = uncheckedUnmaskName(maskedName) ensures
    (
      ...
      "Masking an unmasked name should produce original masked name" + "\n Original masked name " + maskedName +
        ...
        uncheckedMaskName(Variable(uncheckedUnmaskName(maskedName)._1, uncheckedUnmaskName(maskedName)._2, Real)),
    )
  ...
  private[tools] def uncheckedUnmaskName(maskedName: String): (String, Option[Int]) = {
    ...
    if (uscoreMaskedName.startsWith(NAMESPACE_PREFIX)) {
      ...
      "Name conversion of unprefixed (" + NAMESPACE_PREFIX + ") names not allowed: " + uscoreMaskedName
    )
  }
}
